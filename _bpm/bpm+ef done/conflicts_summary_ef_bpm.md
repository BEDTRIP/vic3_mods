...
Отдельно важно: есть ключи, где **семантика у модов разная**, из‑за чего UI‑строки E&F могут выглядеть “обрезанными” или “не про то”:
- `NO_LABOR_MOVEMENT` (у E&F это UI-подсказка про недоступную технологию, у BPM — название модификатора)
- `RURAL_POP` (у E&F это “Agricultural Employment”, у BPM — “Rural Pops”)
- `MARGINAL_TITLE_ACCORDION` (у E&F версия с числом, у BPM без него)
- `RADICALS_TOOLTIP` / `LOYALISTS_TOOLTIP` (BPM-версия короче)

Что делать в компаче: после BPM переопределить эти ключи (как минимум для `english`) в отдельном файле локализации компача (см. `localization/english/zz_ef_bpm_localization_compat_l_english.yml`).
