...
def gen_bank(ef: Path, private: bool) -> tuple[str, str | None]:
    ...
    if private:
        ...
        ghead = (BANNER +
                 ...
                 "###\n"
                 "### EMITTED ONLY UNDER --private-bank, and the default is off ON PURPOSE.\n"
                 "###\n"
                 "### is_government_funded means the treasury pays the building's inputs and\n"
                 "### wages and takes its output. Off, the central bank is an ordinary private\n"
                 "### business, and on a 1836 British save it makes 200 bonds plus currency\n"
                 "### against about 2.5K of inputs and wages. As a government building that gap\n"
                 "### is state spending; as a private one it is profit, dividends and\n"
                 "### investment-pool inflow, every month, for whoever owns the central bank.\n"
                 "###\n"
                 "### That is not the mod this is. The company holds the central bank so it can\n"
                 "### mint the country's prestige currency and so nobody else can buy the bank\n"
                 "### out from under it -- not so it can collect the bank's profits. Leaving\n"
                 "### this file out is why the building panel calls the central bank a\n"
                 "### government building while a company sits on its ownership tab, and that\n"
                 "### is the panel telling the truth about where the money goes.\n\n")
        group = ghead + g + "\n"
    return bank, group
...
def gen_upkeep(ef: Path, names: list[str]) -> dict[str, str]:
    ...
    effects = (BANNER +
               ...
               "\t###\n"
               "\t### The membership test is \"does a central bank stand here\", not\n"
               "\t### has_modifier = has_central_bank. The modifier is E&F's bookkeeping and it\n"
               "\t### is applied and removed on its own pulses; a country whose bank is between\n"
               "\t### pulses would drop out of the denominator for a month, the remaining\n"
               "\t### issuers' shares would jump, and they would all overprint until it came\n"
               "\t### back. On a market with one issuer that is invisible -- its share is 1\n"
               "\t### either way -- which is exactly the shape of what is left: single-issuer\n"
               "\t### markets went quiet, Britain and France did not.\n"
...
def gen_ownership(ef: Path, names: list[str]) -> tuple[dict[str, str], list[str]]:
    ...
    dispatch = (BANNER +
                ...
                "\t\t\t}\n"
                "\t\t}\n\n"
                "\t\t### A rebuilt bank comes back bare, and both of E&F's setup effects have to\n"
                "\t\t### run or it spends a month printing nothing.\n"
                "\t\t###\n"
                "\t\t### central_bank_production_methods picks the methods. central_bank_modifier\n"
                "\t\t### is the one that was missing, and it is where the money actually comes\n"
                "\t\t### from: it finds the bank, takes the base_demande_currency_fix snapshot and\n"
                "\t\t### re-applies `currency_demande`, the mult that turns the production\n"
                "\t\t### method's ~27 units into Britain's 100K. The modifier lives ON THE\n"
                "\t\t### BUILDING, so tearing the building down takes it with it.\n"
                "\t\t###\n"
                "\t\t### That is the annual crash in the currency price. E&F calls its spawners\n"
                "\t\t### from ef_on_yearly_pulse_country, and Victoria 3 spreads a yearly pulse\n"
                "\t\t### across the days of the year -- so every country has its own date and\n"
                "\t\t### hits it again every year. West Bengal on 1 January two years running,\n"
                "\t\t### China in April. Whenever gdp_view had grown enough to ask for a bigger\n"
                "\t\t### bank, the rebuild dropped that country's whole issuance for a month and\n"
                "\t\t### the market price went to 0.01 until the next pulse put the modifier\n"
                "\t\t### back.\n"
                "\t\towner = {\n"
...
def main() -> int:
    ...
    ap.add_argument("--private-bank", action="store_true",
                    help="also drop is_government_funded from bg_bank, so the central bank's "
                         "profits go to the company that owns it instead of the treasury -- "
                         "off by default, see the file it emits")
