...
пара: LLWA × E&F
статус: done
версии: Game 1.13 (exe 1.13.11) — Locomotion (LLWA) 2.6.3, Economic and Financial Mod (E&F) - V4.
позиция: —
файлов: 4
...
* **`LLWA_building_airway` and `_llwa/llwa+morg out`.** That third-party compatch (see `_llwa/llwa+morg+ef done`) empties `LLWA_building_airway` to `{}` and redirects its role onto `building_airport` instead ("Aerodromes? GONE!" in its own source). When both compatches run, this patch's `TRY_INJECT:` still lands a `production_method_groups` on that now-empty stub — harmless (the building has no `building_group`/texture/etc. left either way and can't be built) but pointless in that specific combination. `TRY_INJECT:` is still the right prefix — it's what keeps this patch from logging an error if a *future* LLWA version removes the building outright.
