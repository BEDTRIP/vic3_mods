...
статус: done, переписан под E&F 02.09.2026 (EF.1-EF.8), торговля/формула/престиж
  возвращены на liquidity_currency (EF.9, тем же вечером) — см. План проекта.md
версии: —
позиция: —
файлов: 28
...
плоских 2500. `local_currency` (страны без монетной системы) и
`liquidity_currency` (три престижные валюты ЦБ) — теперь разные товары,
так что рыночная утечка, из-за которой формула была убрана в EF.5, больше не
относится к `liquidity_currency`: он торгуем сознательно, `local_currency`
остаётся нет-торгуемым. Подробности — раздел EF.9 в `План проекта.md`.
