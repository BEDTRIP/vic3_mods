...
| PBE + TGR | `done` — мерджится один ключ `force_regime_change` (`common/diplomatic_actions/`). Компач переписан 2026-08-21 после переименования `kates_` → `vokaes_`: старая версия ссылалась на несуществующие game rule и script value, ветка infamy молча не работала. Структура PBE + числа TGR, восстановлена потерянная PBE `add_cohesion_percent`. Два общих `modifier_type_definitions` (`country_influence_add`, `state_bureaucrats_investment_pool_contribution_add`) различаются только `decimals`/`color` — пин удалён. См. `_pbe/pbe+tgr done/conflicts_tgr_vs_pbe_report.md` | 2026-08-21 |
| PBE + VC | `noneed` | — |
