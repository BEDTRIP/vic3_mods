...
- **`zz_PSC_construction.txt` не переопределяет `has_max_level`** в `REPLACE:building_construction_sector`. При партиальной семантике `REPLACE:` (см. `сводка_ef.md`, «REPLACE: работает по под-блокам», подтверждено 21.08.2026) это значит, что флаг наследуется от предыдущего слоя (в ваниле и в TGR он `yes`) и не обнуляется. Полезно помнить при проверке PSC против любого другого мода, который тоже трогает `building_construction_sector`: реально теряются только те поля, которые чужой мод меняет, а PSC тоже перечисляет (`levels_per_mesh`, `ai_value`, `background`, `required_construction`).
- Компач `psc+tgr noneed` пересверен, конфликта по-прежнему нет. Подробности — `_psc/psc+tgr_analysis_2026-08-21.md`.
